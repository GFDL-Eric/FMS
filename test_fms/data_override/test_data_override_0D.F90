...
integer                           :: id_x, id_y, id_z, id_lon, id_lat, id_co2
integer                           :: i, j, k, is, ie, js, je, unit, io, ierr, n
real                              :: co2_before, co2
character(len=36)                 :: message
type(time_type)                   :: Time
logical                           :: used
logical, allocatable              :: ov_co2(:)
...
id_co2 = register_diag_field  ('test_data_override_mod', 'co2', (/1/), Time, 'CO2', 'ppmv')
...
print *, "co2 before override", co2
print *, "co2 checksum before override", mpp_chksum(co2)
...
!$OMP PARALLEL
!$ call fms_affinity_set("test_data_override_0D", .FALSE., omp_get_num_threads() )
!$OMP END PARALLEL
...
!   jsw = js_win(n)
!   jew = jsw + ny_win - 1
   call data_override('ATM','co2',co2,Time,override=ov_co2(n))
enddo


if(ANY(.NOT. ov_co2)) then
  message = 'override failed for co2'
  call error_mesg('test_data_override_0D', trim(message), FATAL)
endif

print *, "co2 after override", co2
print *, "co2 checksum after override", mpp_chksum(co2)

if(co2 == co2_before) then
  call error_mesg('test_data_override_0D', 'sums before and after override are equal', FATAL)
endif

if(id_co2 > 0) used = send_data(id_co2, co2, Time)
